function [image_lbp, counts] = lbp_inter(image, invariant, radio, num_points)
%LBP_INTER Returns a MxN local binary pattern (LBP) image and its histogram.
%   Every LBP label is computed using interpolated neighbors from a neighborhood
%   of variable size "radio "and number of points "num_points".
%
%   LBP_INTER(I, 0, 1, 8)  returns a LBP image from "image". Neighbor pixels
%   are computed from interpolated values. In addition, "counts" is the 
%   column LBP histogram. 
%
%   LBP_INTER(I, 1, 1, 8) returns a LBP image from "image". Neighbor pixels
