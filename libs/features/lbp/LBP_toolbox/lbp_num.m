function image_lbp = lbp_num(image, radio, num_points)
%LBP_NUM Returns a MxN number local binary pattern (LBP) image. Every LBP 
%   label is computed using interpolated neighbors from a neighborhood
%   of variable size "radio "and number of points "num_points".
%
%   LBP_NUM(I, 1, 8) returns a number LBP image where every LBP label is 
%   computed using the number measure propose by Ma (Number Local Binary 
%   Pattern). 
...
if bin2dec_value <= 2
    ...
    numbered_lbp = sum(diff_values);
    
    if numbered_lbp == 4
        bin2dec_value = 4;
    elseif numbered_lbp > 4
        bin2dec_value = numbered_lbp;
    else
        bin2dec_value = 8 - numbered_lbp;
    end
